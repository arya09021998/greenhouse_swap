...
type PublicFarmData = {
  ...
  depositFee: string
  withdrawFee: string
  chronicPerBlock: SerializedBigNumber
}
...
const fetchFarm = async (farm: Farm): Promise<PublicFarmData> => {
  ...
  const [info, totalAllocPoint, chronicPerBlock] =
    pid || pid === 0
  ...
  const ChronicPerBlock = chronicPerBlock ? new BigNumber(chronicPerBlock).div(BIG_TEN.pow(18)) : BIG_ZERO
  ...
  return {
    ...
    chronicPerBlock: ChronicPerBlock.toJSON(),
  }
}
